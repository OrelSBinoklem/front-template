...
module.exports = function(config) {
    if(config.imgCompressPolicy == "good" && !fs.existsSync(path.join(process.cwd(), config.src, '/img/simple'))) {
        mkdirp(path.join(process.cwd(), config.src, '/img/simple'), function (err) {
            if (err) console.error(err)
            else console.log('create folder ./img/simple');
        });
    } else if(config.imgCompressPolicy == "simple" && !fs.existsSync(path.join(process.cwd(), config.src, '/img/good'))) {
